...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const openState = {};      // catKey -> boolean
  ...
  function renderCategories(query = "") {
    ...
    MENU.forEach(cat => {
      ...
      wrap.className = `rounded-2xl border border-gray-200 overflow-hidden bg-white border-l-4 ${ui.rail}`;
        ...
        card.innerHTML = `
          <div class="flex items-start justify-between gap-2">
            ...
            <button type="button"
              ...
              data-remove="${esc(x.id)}">Убрать</button>
          </div>
        `;
  ...
  if (btnNext) btnNext.addEventListener("click", () => {
    ...
    showStep(Math.min(current + 1, LAST_STEP));
  });
...
`;
  ...
  if (window.emailjs && EMAILJS_PUBLIC_KEY) {
    try { emailjs.init({ publicKey: EMAILJS_PUBLIC_KEY }); } catch (_) { /* ignore */ }
  }
  ...
  form.addEventListener("submit", (e) => {
    ...
    // Если Wizard открыт: Step 1 должен быть валиден
    if (wizardOpen) {
