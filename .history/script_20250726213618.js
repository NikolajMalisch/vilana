...
// Event-Listener für das Klicken auf Menü-Kategorien
document.querySelectorAll(".menu-tab").forEach((tab) => {
  tab.addEventListener("click", function () {
    const category = this.dataset.category;
    showMenuCategory(category);
...
// Event-Listener für das Klicken auf mobile Menü-Items
document.querySelectorAll(".mobile-menu-item").forEach((item) => {
  item.addEventListener("click", function () {
    const category = this.dataset.category;
    showMenuCategory(category);
...
// Event-Listener für das Klicken außerhalb des mobilen Menüs
document.addEventListener("click", function (event) {
  const menu = document.getElementById("mobile-menu");
  if (!menu.contains(event.target) && !document.getElementById("mobile-menu-toggle").contains(event.target)) {
    menu.classList.add("hidden");
...
// Event-Listener für Escape-Taste zum Schließen des mobilen Menüs